...
class UnifiedPanel(QMainWindow):
    ...
    def handle_viewer_click(self, video_x, video_y, frame_idx):
        fence_mode_active = any(f.get('mode', False) for f in self.fences)
        
        if fence_mode_active:
            for fence in self.fences:
                if fence.get('mode', False):
                    fence['points'].append((video_x, video_y))
                    break
            if self.viewer:
                self.viewer.update_display()
            return
        ...
        for ann in filtered:
            ...
            if cv2.pointPolygonTest(pts, (float(video_x), float(video_y)), False) >= 0:
                track_id = ann.get('track_id', ann.get('id', 0))
                self.del_points.append({'x': video_x, 'y': video_y, 'frame_idx': frame_idx, 'track_id': track_id})
